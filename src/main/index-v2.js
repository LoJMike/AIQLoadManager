...
function track(event, props = {}) {
  try {
    if (!posthog || !_store || _store.get('analytics.enabled') === false) return;
    posthog.capture({
...
const PRESET_TEXTS = {
  concise: 'Respond as concisely as possible. No preamble, no padding, no filler. Get straight to the point.',
  caveman: 'CAVEMAN MODE: Only use short simple words. Short sentences. No big words. No long explanations. Direct answer only.',
  bullet:  'Respond using bullet points only. Never write prose paragraphs. Each bullet must be one idea, one line.',
 ...
 */
function generateDigestHtml(items, { title = 'AIQ Load Manager — Session Digest' } = {}) {
...
app.whenReady().then(() => {
  ...
  for (const [provider, model] of Object.entries(savedDefaultModels)) {
    try { registry.setDefaultModel(provider, model); } catch (_) {}
  }
...
function setupIPC() {
  ...
  ipcMain.handle('add-to-queue', (_, item) => {
    ...
    const result = queue.addItem({ ...item, tags, taskType, priority, compareProviders });
    track('prompt_queued', {
      routing_mode:  item.routingMode || 'auto',
  ...
  ipcMain.handle('get-standing-instructions', () => store.get('standingInstructions') || '');
  ipcMain.handle('set-standing-instructions', (_, text) => {
    const trimmed = (text || '').trim();
    if (trimmed) {
      store.set('standingInstructions', trimmed);
    } else {
      store.delete('standingInstructions');
    }
  ...
  ipcMain.handle('get-provider-styles', () => store.get('responseStyles') || {});

  ipcMain.handle('set-provider-style', (_, { provider, preset, customText }) => {
    const styles = store.get('responseStyles') || {};
    if (!preset || preset === 'normal') {
      delete styles[provider];
    } else {
      styles[provider] = { preset, customText: customText || '' };
    }
    store.set('responseStyles', styles);
  ...
  ipcMain.handle('get-default-models', () => store.get('defaultModels') || {});

  ipcMain.handle('set-default-model', (_, { provider, model }) => {
    const lic = license.getLicense();
    if (!lic.flags.modelControl) {
      throw new Error('Default model control requires a Pro license or higher.');
    }
    const defaults = store.get('defaultModels') || {};
    if (model) {
      defaults[provider] = model;
    } else {
      delete defaults[provider];
    }
    store.set('defaultModels', defaults);
    registry.setDefaultModel(provider, model || null);
    return { success: true };
  });

  // ── Project history ────────────────────────────────────────────────────────
  ipcMain.handle('get-project-history', (_, projectId) => queue.getProjectHistory(projectId));
  ...
  ipcMain.handle('export-digest', async (_, opts = {}) => {
    ...
    const html  = generateDigestHtml(items, { title: opts.title || 'AIQ Load Manager — Session Digest' });
  ...
  // Analytics opt-out
  ipcMain.handle('get-analytics-enabled', () => _store.get('analytics.enabled') !== false);
